...
typedef struct _NDIS_WAN_GET_STATS_INFO
{
    ...
    _Out_ ULONG       CRCErrors;                      // Serial-like info only
    _Out_ ULONG       TimeoutErrors;                  // Serial-like info only
    _Out_ ULONG       AlignmentErrors;                // Serial-like info only
    _Out_ ULONG       SerialOverrunErrors;            // Serial-like info only
    _Out_ ULONG       FramingErrors;                  // Serial-like info only
    _Out_ ULONG       BufferOverrunErrors;            // Serial-like info only
    _Out_ ULONG       BytesTransmittedUncompressed;   // Compression info only
    _Out_ ULONG       BytesReceivedUncompressed;      // Compression info only
    _Out_ ULONG       BytesTransmittedCompressed;     // Compression info only
    _Out_ ULONG       BytesReceivedCompressed;        // Compression info only
